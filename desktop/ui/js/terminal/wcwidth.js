 ...
 *
 * A direct port of the Android app's `WcWidth.kt`, tables and all, so the two
 * clients lay out the same output identically. Tables are sorted inclusive
 * ranges (Unicode 15) searched with binary search.
 */
...
const ZERO = Int32Array.from([
  ...
  0x200B, 0x200F, 0x2028, 0x202E, 0x2060, 0x2064, 0x2066, 0x206F, 0x20D0, 0x20F0,
...
const WIDE = Int32Array.from([
  ...
  0xFE54, 0xFE66, 0xFE68, 0xFE6B, 0xFF01, 0xFF60, 0xFFE0, 0xFFE6, 0x16FE0, 0x16FE3,
...
function inRanges(cp, table) {
  ...
  while (lo <= hi) {
    const mid = (lo + hi) >>> 1;
    const start = table[mid * 2];
    const end = table[mid * 2 + 1];
...
export function isCombining(cp) {
  return cp >= 0x300 && inRanges(cp, ZERO);
}
